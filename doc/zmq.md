...
The SmartLoopAI Core daemon can be configured to act as a trusted "border
router", implementing the smartloopai wire protocol and relay, making
...
The ZeroMQ feature in SmartLoopAI Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building smartloopaid:
...
For instance:
    ...
    $ smartloopaid -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/smartloopaid.tx.raw \
...
These options can also be provided in smartloopai.conf.
...
From the perspective of smartloopaid, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into smartloopaid directly. Furthermore, no information is
...
using. SmartLoopAId appends an up-counting sequence number to each
