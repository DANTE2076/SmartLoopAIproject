# SmartLoopAI Core version v20.0.0
...
Currently, 10% of block rewards are set aside for the SmartLoopAI DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking SmartLoopAI in a
credit pool. This credit pool mirrors the available credit supply in SmartLoopAI
Platform. When a certain amount of SmartLoopAI is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on SmartLoopAI
Platform. Masternode identities receive the fees and can convert them back to
SmartLoopAI by withdrawing them from SmartLoopAI Platform.
...
- `bestCLHeightDiff` (`uint32`) is equal to the diff in heights from the mined
  block height
- `creditPoolBalance` (`int64`) is equal to all the smartloop locked in all previous
  blocks minus all the smartloop unlocked in all previous blocks plus all the block
...
### Removal of reject network messages from SmartLoopAI Core (BIP61)

This feature has been disabled by default since SmartLoopAI Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since SmartLoopAI Core version 20.0.0 there are extra changes:
...
- It is possible to run SmartLoopAI Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `SMLP/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from SmartLoopAI Core (BIP61)_](#removal-of-reject-network-messages-from-smartloopai-core-bip61) above for
...
- The `smartloopai-cli -getinfo` command now displays the wallet name and balance for
...
python test framework and `smartloopai-cli` tool both take advantage of this, so
for example:

```sh
smartloopai-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
smartloopai-cli -named createwallet mywallet load_on_startup=1
```
...
with SmartLoopAI’s product needs, such as SegWit and RBF, are excluded from our
...
- [v19.3.0](https://github.com/SmartLoopAIproject/blob/master/doc/release-notes/smartloopai/release-notes-19.3.0.md) released July/31/2023
- [v19.2.0](https://github.com/SmartLoopAIproject/blob/master/doc/release-notes/smartloopai/release-notes-19.2.0.md) released June/19/2023
- [v19.1.0](https://github.com/SmartLoopAIproject/blob/master/doc/release-notes/smartloopai/release-notes-19.1.0.md) released May/22/2023
- [v19.0.0](https://github.com/SmartLoopAIproject/blob/master/doc/release-notes/smartloopai/release-notes-19.0.0.md) released Apr/14/2023
- [v18.2.2](https://github.com/SmartLoopAIproject/blob/master/doc/release-notes/smartloopai/release-notes-18.2.2.md) released Mar/21/2023
- [v18.2.1](https://github.com/SmartLoopAIproject/blob/master/doc/release-notes/smartloopai/release-notes-18.2.1.md) released Jan/17/2023
...
[DIP-0023]: https://github.com/smartloopaipay/dips/blob/master/dip-0023.md
[DIP-0029]: https://github.com/smartloopaipay/dips/blob/master/dip-0029.md

[i2p]: https://github.com/SmartLoopAIproject/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/SmartLoopAIproject/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/SmartLoopAIproject/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/SmartLoopAIproject/compare/v19.3.0...smartloopaipay:v20.0.0
